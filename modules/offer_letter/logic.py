"""
modules/offer_letter/logic.py

Core business logic for the Offer Letter module:
...
def generate_offer_letter(template_bytes, title, name, role, location, phone, address, offer_date, joining_date, hr_name, hr_dept):
    """
    Wrapper function to build replacements and fill the DOCX template.
    Returns a dictionary formatted for the Streamlit UI.
    """
    try:
        # 1. Build the replacements map
        full_name, replacements = build_replacements(
            title, name, role, location, phone, address, offer_date, joining_date, hr_name, hr_dept
        )
        
        # 2. Fill the DOCX template
        docx_bytes = fill_offer_letter(template_bytes, replacements)
        
        # 3. Create a safe filename for the download/attachment
        safe_name = name.strip().replace(" ", "_")
        filename = f"{safe_name}_Offer_Letter.docx"
        
        # 4. Build the summary data for the UI Preview tab
        preview_data = {
            "Candidate": full_name,
            "Role": role,
            "Location": location,
